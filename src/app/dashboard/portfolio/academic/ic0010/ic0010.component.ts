...
import { IC0010 } from 'core/interfaces/Portfolio/Academic/IC0010';
...
@Component({
  selector: 'app-ic0010',
  templateUrl: './ic0010.component.html',
  styleUrls: ['./ic0010.component.css']
})
export class IC0010Component implements OnInit, OnChanges, OnDestroy {

  errorMessage: string;
  container_id = 'c14826bf-834f-4d23-b554-1bab9fcecf9f';
  container_name = 'PSAT 10';
  errorGet = 'Cannot Get ' + this.container_name;
  errorUpdate = 'Cannot Update ' + this.container_name;
  @Input() container: IC0010;
  @Output() EventOutput: EventEmitter<IC0010> = new EventEmitter<IC0010>();
  ...
  updateContainer() {
    ...
    putRequest.container_code = 'IC0010';
      ...
      .subscribe(response => {
        ...
        if (this.apiResponse.response.code === 'AM0000') {
          if (this.apiResponse.container) {
            try {
              this.container = <IC0010>JSON.parse(this.apiResponse.container.json);
            } catch (error) {
              console.log(error);
            }
          }
  ...
  open(isnew, position, content) {
    ...
    if(isnew){
        this.title_modal = "Add New PSAT 10";
        this.index = -1;
    }else{
    this.title_modal = "Update PSAT 10";
    this.index = position;
    }

    if(position >= 0){
        let psat_10 = this.container.psat_10[position];
        this.evidence_based = psat_10.evidence_based;
        this.reading = psat_10.reading;
        this.writing_language = psat_10.writing_language;
        this.math = psat_10.math;
    }
  ...
  save_psat_10(content):boolean{
    this.show_alert = false;
    
    if(this.FieldsValidation()){
      //Se ingresa la info obtenida del modal al listado de profesiones
      if(this.index >= 0){
        this.container.psat_10[this.index].evidence_based = this.evidence_based;
        this.container.psat_10[this.index].reading = this.reading;
        this.container.psat_10[this.index].writing_language = this.writing_language;
        this.container.psat_10[this.index].math = this.math;
      }else{
        this.container.psat_10.push({
  ...
  removeItem(position):void{
    var element = document.getElementById("psat_10-"+position).classList.add("remove-item");

    setTimeout(() => {
      this.container.psat_10.splice(position, 1);
      this.updateContainer();
    }, 600);
  }
}