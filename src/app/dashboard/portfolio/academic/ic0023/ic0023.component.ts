...
import { IC0023 } from 'core/interfaces/Portfolio/Academic/IC0023';
...
@Component({
  selector: 'app-ic0023',
  templateUrl: './ic0023.component.html',
  styleUrls: ['./ic0023.component.css']
})
export class IC0023Component implements OnInit, OnChanges, OnDestroy {
  public value = false;
  container_tooltip = "Academic Recommendations.";
  container_name= "Academic Recommendations";

  errorMessage: string;
  container_id = '9ea86f69-c37e-4196-afcb-9e67e5cdc95a';
  @Input() container: IC0023;
  @Output() EventOutput: EventEmitter<IC0023> = new EventEmitter<IC0023>();
  ...
  updateContainer() {
    ...
    putRequest.container_code = 'IC0023';
      ...
      .subscribe(response => {
        ...
        if (this.apiResponse.response.code === 'AM0000') {
          if (this.apiResponse.container) {
            try {
              this.container = <IC0023>JSON.parse(this.apiResponse.container.json);
            } catch (error) {
              console.log(error);
            }
          }
